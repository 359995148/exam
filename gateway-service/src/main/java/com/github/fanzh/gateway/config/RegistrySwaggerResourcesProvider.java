...
public class RegistrySwaggerResourcesProvider implements SwaggerResourcesProvider {
    ...
    public List<SwaggerResource> get() {
        ...
        List<Route> routeList = this.listRoute();
        if (ParamsUtil.isEmpty(routeList)) {
            return resources;
        }
        for (Route route : routeList) {
            if (routes.contains(route.getRouteId())) {
                try {
                    ...
                    if (ParamsUtil.isEmpty(routePredicateVoList)) {
                        continue;
                    }
                    String genKey;
                    ...
                    if (ParamsUtil.isEmpty(routePredicateVo)) {
                        continue;
                    }
    ...
    private List<Route> listRoute() {
        ...
        if (ParamsUtil.isEmpty(object)) {
            // 放入缓存
            routeList = routeService.selectList(null);
            if (ParamsUtil.isNotEmpty(routeList)) {
