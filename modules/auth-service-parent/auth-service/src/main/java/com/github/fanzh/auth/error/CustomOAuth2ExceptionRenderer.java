package com.github.fanzh.auth.error;
...
public class CustomOAuth2ExceptionRenderer {
	...
	public void handleResponseBeanResponse(ResponseEntity<?> responseEntity, ServletWebRequest webRequest)
		...
		if (body != null) {
			// 返回ResponseBean
			ExecResult<?> execResult = new ExecResult<>(body, ApiMsg.KEY_ACCESS, ApiMsg.DENIED);
			writeWithMessageConverters(execResult, inputMessage, outputMessage);
		} else {
			// flush headers
